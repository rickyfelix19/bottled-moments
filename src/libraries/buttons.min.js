...
const STYLE_HOVER = { background: "#686DE0", color: "#FBF4E9" };
const STYLE_PRESSED = { background: "#686DE0", color: "#FBF4E9" };
...
class Button {
  ...
  #calculateBounds() {
    ...
    (this.#bounds.minx = this.#props.x + offset_x),
      ...
      (this.#bounds.centery =
        this.#props.y + this.#props.height / 2 + offset_y);
  }
  #calculateStyles() {
    this.#cstyles.default = Object.assign(
      {},
      STYLE_DEFAULT,
      this.#props.style_default
    );
    this.#cstyles.hover = Object.assign(
      {},
      STYLE_DEFAULT,
      this.#props.style_default,
      this.#props.style_hover
    );
    this.#cstyles.pressed = Object.assign(
      {},
      STYLE_DEFAULT,
      this.#props.style_default,
      this.#props.style_pressed
    );
    this.#cstyles.disabled = Object.assign(
      {},
      STYLE_DEFAULT,
      this.#props.style_default,
      this.#props.style_disabled
    );
  }
  update(properties) {
    try {
      ...
      console.warn(
        `Encountered an unrecognized property!Original error:${e.message}`
      );
    }
    if (
      "x" in properties ||
      ...
      "align_x" in properties ||
      "align_y" in properties
    )
      this.#calculateBounds();
    if (
      "style_default" in properties ||
      "style_hover" in properties ||
      "style_pressed" in properties ||
      "style_disabled" in properties
    )
      this.#calculateStyles();
  }
  ...
  draw(context = globalThis) {
    ...
    if (!this.#was_pressed && is_pressed && this.#props.on_press)
      this.#props.on_press();
    if (this.#was_pressed && !is_pressed && this.#props.on_release)
      this.#props.on_release();
    if (!this.#was_hovering && is_hovering && this.#props.on_mouse_enter)
      this.#props.on_mouse_enter();
    if (this.#was_hovering && !is_hovering && this.#props.on_mouse_exit)
