...
(function () {
  ...
  */
      ...
      switch (this.shape) {
        case "rect":
          ...
          this.inputObject = (input) => {
            if (
              checkRectHover(this.x, this.y, this.w, this.h, mouseX, mouseY) &&
              input === this.kindOfInput
            ) {
              this.callback();
            }
          };
        ...
        case "square":
          ...
          this.inputObject = (input) => {
            if (
              checkRectHover(this.x, this.y, this.s, this.s, mouseX, mouseY) &&
              input === this.kindOfInput
            ) {
              this.callback();
            }
        ...
        case "circle":
          ...
          this.inputObject = (input) => {
            if (
              checkCircleHover(this.x, this.y, this.d, mouseX, mouseY) &&
              input === this.kindOfInput
            ) {
              this.callback();
            }
  ...
  function checkTriangleHover(x1, y1, x2, y2, x3, y3, xP, yP) {
    let totalArea = Math.abs(
      (x1 * (y2 - y3) + x2 * (y3 - y1) + x3 * (y1 - y2)) / 2
    );
    let firstArea = Math.abs(
      (xP * (y1 - y2) + x1 * (y2 - yP) + x2 * (yP - y1)) / 2
    );
    let secondArea = Math.abs(
      (xP * (y2 - y3) + x2 * (y3 - yP) + x3 * (yP - y2)) / 2
    );
    let thirdArea = Math.abs(
      (xP * (y1 - y3) + x1 * (y3 - yP) + x3 * (yP - y1)) / 2
    );
  ...
  function checkEllipseHover(x, y, w, h, xP, yP) {
    ...
    let equ =
      Math.pow(xP - x, 2) / Math.pow(w, 2) +
      Math.pow(yP - y, 2) / Math.pow(h, 2);
